...
#pragma warning restore S125 // Sections of code should not be commented outs
        ...
        public async Task<int> DeleteRangeAsync(Expression<Func<TEntity, bool>> whereExpression, bool logicDelete = true, CancellationToken cancellationToken = default)
        {
            var enityType = typeof(TEntity);
            var hasSoftDeleteMember = typeof(ISoftDelete).IsAssignableFrom(enityType);
            if (hasSoftDeleteMember && logicDelete)
            {
